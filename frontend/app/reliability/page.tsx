...
export default function Reliability() {
  ...
  async function load() {
    try { const [nextImpacts, nextAssertions] = await Promise.all([api<any[]>(`/api/projects/${project}/change-impacts`), api<any[]>(`/api/projects/${project}/assertions`)]); setImpacts(nextImpacts); setAssertions(nextAssertions); }
  ...
  const unresolved = assertions.filter(item => ["proposed", "possibly_stale", "stale", "contradicted"].includes(item.status));
  ...
  return <Page title="Runbook Reliability" description="CI/CD for production runbooks: review the evidence when code or configuration changes may make a procedure stale." action={<div className="row"><select value={project} onChange={event => setProject(event.target.value)}>{projects.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button className="button secondary" onClick={analyze} disabled={!project || busy}>{busy ? "Refreshing…" : "Refresh repository"}</button></div>}>
    ...
    <section className="card" style={{marginTop:16}}><div className="section-head"><h2>Operational assertions</h2><span className="badge">{assertions.length}</span></div><div className="card-pad stack">{assertions.length ? assertions.slice(0,8).map(assertion => <div className="source" key={assertion.id}><div className="row between"><strong>{assertion.title}</strong><span className={`badge ${tone[assertion.status] || "info"}`}>{assertion.status.replace(/_/g," ")}</span></div><p>{assertion.claim}</p><span className="subtle">Reviewer: {assertion.verification_owner || "owner unknown"} · {assertion.environment_scope}</span></div>) : <p className="subtle">Assertions are created when a runbook is extracted from cited evidence.</p>}</div></section>
  </Page>;
}
